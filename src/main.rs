use std::{env, net::SocketAddr, time::{Duration, SystemTime, UNIX_EPOCH}};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let origin: HeaderValue = env::var("CORS_ORIGIN")
        .unwrap_or_else(|_| "http://localhost:5173".to_owned()).parse()?;
    let app = Router::new()
...
fn token_for(user_id: i32, secret: &str) -> Result<String, ApiError> {
    let now = SystemTime::now().duration_since(UNIX_EPOCH)
        ...
        .as_secs();
    let claims = Claims { user_id, exp: now + 7 * 24 * 60 * 60 };
...
fn authenticated_user(headers: &HeaderMap, secret: &str) -> Result<i32, ApiError> {
    ...
    let token = authorization.strip_prefix("Bearer ")
        .ok_or(ApiError(StatusCode::UNAUTHORIZED, "Unauthorized"))?;
    let claims = decode::<Claims>(token, &DecodingKey::from_secret(secret.as_bytes()),
...
async fn signup(State(state): State<AppState>, Json(input): Json<Credentials>)
    -> Result<Json<TokenResponse>, ApiError>
{
    let email = input.email.trim().to_lowercase();
    if email.len() > 254 || !email.contains('@') || input.password.len() < 8
    ...
    let hash = tokio::task::spawn_blocking(move || bcrypt::hash(input.password, 10))
        .await
        .map_err(|_| ApiError(StatusCode::INTERNAL_SERVER_ERROR, "Failed to create account"))?
        .map_err(|_| ApiError(StatusCode::INTERNAL_SERVER_ERROR, "Failed to create account"))?;
    let user_id: i32 = sqlx::query_scalar(
        "INSERT INTO users (email, password) VALUES ($1, $2) RETURNING id")
        .bind(email).bind(hash).fetch_one(&state.db).await
        .map_err(|error| {
            ...
            ApiError(StatusCode::INTERNAL_SERVER_ERROR, "Failed to create account")
        })?;
    Ok(Json(TokenResponse { token: token_for(user_id, &state.jwt_secret)? }))
}

async fn login(State(state): State<AppState>, Json(input): Json<Credentials>)
    -> Result<Json<AuthResponse>, ApiError>
{
    let email = input.email.trim().to_lowercase();
    let user: Option<User> = sqlx::query_as("SELECT id, password FROM users WHERE lower(email) = $1")
    ...
    let verified = tokio::task::spawn_blocking(move || bcrypt::verify(input.password, &user.password))
        .await
    ...
    Ok(Json(AuthResponse { token: token_for(user.id, &state.jwt_secret)?, user_id: user.id }))
}

async fn summarize(State(state): State<AppState>, headers: HeaderMap,
    Json(input): Json<SummarizeRequest>) -> Result<Json<SummarizeResponse>, ApiError>
{
    ...
    if !response.status().is_success() {
        ...
        return Err(ApiError(StatusCode::BAD_GATEWAY, "Summarization service is unavailable"));
    }
    let completion: CompletionResponse = response.json().await.map_err(|error| {
        ...
        ApiError(StatusCode::BAD_GATEWAY, "Summarization service returned an invalid response")
    })?;
    let summary = completion.choices.into_iter().next()
        .and_then(|choice| choice.message.content)
        .filter(|content| !content.trim().is_empty())
        .ok_or(ApiError(StatusCode::BAD_GATEWAY,
            "Summarization service returned an empty summary"))?;

    sqlx::query("INSERT INTO summaries (original, summary, user_id) VALUES ($1, $2, $3)")
        .bind(article).bind(&summary).bind(user_id).execute(&state.db).await
        .map_err(|error| {
...
async fn history(State(state): State<AppState>, headers: HeaderMap)
    -> Result<Json<Vec<HistoryItem>>, ApiError>
{
    ...
    Ok(Json(rows.into_iter().map(|row| HistoryItem {
        id: row.id,
        summary: row.summary,
